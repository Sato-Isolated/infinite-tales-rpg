<script lang="ts">
	/**
	 * NarrativeMarkupParser - @ Prefix narrative markup to styled HTML converter
	 * New @ prefix system completely avoids HTML/XML syntax confusion
	 * Supports both new @tag:content format and legacy [tag] format for backwards compatibility
	...
	/**
	 * Parse @ prefix narrative markup into HTML with clean styling
	 * New system: @tag:content instead of [tag]content[/tag]
	 * Completely avoids HTML/XML syntax confusion
	 */
	function parseNarrativeMarkup(text: string): string {
		...
		// 1. Speaker dialogue: @speaker:Name:dialogue
		...
		// 2. Character mentions: Name @char or @char:Name
		parsed = parsed.replace(
			/(\w+)\s+@char\b/g,
		...
		// 7. Line breaks: @br
		parsed = parsed.replace(/@br\b/g, '<div class="divider">~</div>');

		// Legacy fallback: Handle old [tag] format for backwards compatibility
		...
		// Remove unknown legacy tags silently (error-tolerant approach)
