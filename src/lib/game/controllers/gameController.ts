...
function logTimeProgression(minutesPassed: number, explanation?: string): void {
	console.log(`⏰ Temps écoulé: ${minutesPassed}min - ${explanation || 'Temps progressé'}`);
}
...
function logInitialTime(gameTime: GameTime, explanation?: string): void {
	...
	console.log(
		`🎯 Temps initial généré par l'IA: ${gameTime.dayName} ${gameTime.day} ${gameTime.monthName} ${gameTime.year}, ${timeString} (${gameTime.timeOfDay})`
	);
	if (explanation) {
